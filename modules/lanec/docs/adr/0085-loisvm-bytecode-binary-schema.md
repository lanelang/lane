...
Each function-table entry begins with `function_entry_tag:u8`: BytecodeBody is `0x01`, and RuntimeImport is `0x02`. A BytecodeBody entry contains the existing length-delimited body and does not repeat FunctionId, context kind, arities, or result representation outside that body. A RuntimeImport entry contains `abi_major:u32le`, `parameter_count:u32le`, one `RuntimeValueKind:u8` per parameter, one result `RuntimeValueKind:u8`, `symbol_length:u32le`, and exact symbol bytes. RuntimeValueKind tags encode Unit, Bool, Int, Double, String, and Opaque. The entry is implicitly no-context and witness-free. Its symbol is nonempty case-sensitive ASCII without NUL. Function entries have no common byte-length field.

After whole-program optimization and final body formation, the linker emits bytecode bodies first in final deterministic body-list order. It deduplicates runtime imports by the complete descriptor and appends them sorted by symbol bytes, ABI major, parameter kinds, then result kind. FunctionIds are reproducible only for the same compiler version, exact inputs, options, and selected entry; build changes may renumber them. The runtime symbol registry supplies implementations, but loading requires its ABI major, parameter kinds, and result kind to match the serialized descriptor exactly.
...
- Runtime imports carry major version, complete parameter and result kinds, and a nonempty ASCII symbol.
