...
A runtime-import adapter consumes or releases every transferred argument before reporting failure. It then throws a private Wasm exception, optionally carrying only non-Lane diagnostic state required by the execution boundary. A generated Lane function that can hold owned values across a potentially failing call installs an internal cleanup handler. A frame containing only trivial values has no cleanup obligation and therefore propagates the private exception directly without a redundant handler. The handler releases exactly the owners still held by that frame at the throwing program point, then rethrows the same private exception.
...
The Wasm backend derives cleanup ownership state from trusted bytecode ownership operations and representation metadata. Owned references and packed callables use their already-invalid zero representation as the empty-owner state, so their physical local is also the cleanup liveness fact. Consuming such an owner clears that local unless physical-slot coalescing places the result in the same local. An owned erased payload retains a separate liveness bit because zero is a valid erased payload and its layout witness is required for cleanup. This derivation is not a general bytecode verifier and does not add serialized borrow regions or source types. Retain, release, and destructor operations used during cleanup must not throw.
