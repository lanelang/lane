...
1. `linked_program_schema_version:u32le = 5`;
2. the complete LoisVM bytecode section occupying every remaining payload byte.

The current payload has no linked-program section directory, section tags, optional sections, or
...
the current bytecode format and execution implementation rather than serialized
Wasm metadata.

Removing the nested bytecode version and adopting the current-only bytecode
layout sets `linked_program_schema_version` to 5. Lane artifacts are compiler
contracts, so the loader rejects older schemas and asks users to relink instead
of retaining a compatibility decoder.
...
the linked-program schema version, selected entry, table
...
- The current linked-program payload is schema version 5 followed by one bytecode
  section.
- The bytecode section consumes the remainder of the artifact payload.
- The current payload has no section directory or redundant bytecode length.
