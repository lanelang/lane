...
`call_value` and `tail_call_value` accept either callable representation. An immediate identifier must target a function-table entry with no closure context. A closure supplies the required environment for a capturing entry. The trusted bytecode contract establishes this relationship; the VM does not dynamically adapt a mismatched callable. Consuming an immediate identifier has no ARC effect. Consuming a unique closure moves its environment into the callee and frees the closure shell. Consuming a shared closure retains the environment for the callee and releases only the consumed closure owner. Uniqueness selects an optimization path and is not a call-validity precondition.
