...
All six functions have an empty Lane effect row. Allocation and fatal primitive
failure do not introduce a user-handleable algebraic effect.

## Failure semantics

The primitive layer is intentionally partial where a low-level operation has a
dynamic precondition:

| Operation | Failure |
| --- | --- |
| `%bytes_make(length, _)` with `length < 0` | fatal runtime panic |
| `%bytes_make(length, _)` beyond the supported allocation/resource limit | fatal runtime panic |
| `%bytes_get(bytes, index)` with `index < 0` or `index >= length(bytes)` | fatal runtime panic |
| `%bytes_set(bytes, index, _)` with `index < 0` or `index >= length(bytes)` | fatal runtime panic |
| allocation failure | fatal runtime panic |

No operation clamps an index, wraps an index, returns a default byte, or exposes
undefined behavior. `%int_to_byte` is total and never panics because truncation
is its specified behavior.

Safe APIs are ordinary library code. A library may check `%bytes_length` before
calling `%bytes_get` or `%bytes_set` and return `Option`, a nominal error type,
or an effectful result without changing the compiler contract.
...
Both execution tiers implement identical behavior:
...
- both report invalid length, invalid index, resource exhaustion, and
  allocation failure through the existing fatal runtime failure boundary.
...
- Negative and `length`-equal indices panic for get and set.
- Oversized allocation and allocation failure use fatal runtime failure.
...
- Interpreter and Wasm/JIT execution produce identical successful results and
  fatal failures.
...
5. interpreter and Wasm/JIT behavior agree, including panic cases;
