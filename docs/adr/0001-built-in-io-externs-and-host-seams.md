...
`Basic.Io` exports `println : (String) -> Unit ! Io = extern("println")` and `panic : (String) -> Basic.Data.Void.Void ! Panic = builtin("%panic")`. `println` is an ordinary Runtime Import selected by its open host symbol. Panic is closed compiler-owned fatal control: it creates no Runtime Import, requires no host registration, uses the ordinary nominal-data result ABI for canonical Void, and ends its intrinsic wrapper with the `Fatal` terminator specified by the compiler fatal-control RFC. `Panic` owns source observability, while the verified intrinsic contract and `Fatal` own terminal control. A selected executable entry has zero parameters, returns `Unit`, and is validated against an explicit execution profile. The Lane CLI profile admits `Io`, `Panic`, and closed External Effects; residual algebraic or open effects are rejected. Runtime-import resolution and invocation failures remain fatal execution errors rather than Lane effects, but they are distinct from intentional source `Fatal` execution.
...
`mon-trans` is driven by the general monadic-effect predicate: an open row or any handled operation requires translation, and resume counts are never analyzed. Translation preserves non-monadic residual effects such as `Io` and `Panic`; a later residual-effect-erasure pass removes them only after effect-sensitive optimization while preserving observable calls and evaluation order.
