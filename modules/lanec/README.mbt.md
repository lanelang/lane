...
`lanec` stops at execution-image production. To execute generated bytecode directly, use `Milky2018/loisvm`; to use the standard Lane command workflow, use the `lane run`, `lane compile`, `lane link`, and `lane exec` commands from the `Milky2018/lane` module. `run` and `exec` use Wasmoon JIT by default and accept `--no-jit` for interpreter execution.
