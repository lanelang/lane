...
artifact. The artifact boundary encodes the compiler-produced module, reparses
the exact bytes, and validates them before exposing a `VerifiedWasmExecutable`.
Before execution, `lane_runtime/wasm` independently parses and validates the
untrusted external bytes, authenticates host bindings against their actual
import section, resolves named control globals, and constructs the executable
instance atomically. Failure at either trust boundary returns no partially
trusted program.
