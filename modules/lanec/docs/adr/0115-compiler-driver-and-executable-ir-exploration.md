...
A failed compilation produces a Partial Explore Report containing all eighteen ordered Explore Stage entries: every earlier completed stage retains its snapshot, the failing stage owns its diagnostics, and every later stage is unavailable. The report's typed overall status identifies the same failed stage. Entry selection is part of the `buslane.linked` boundary, so a missing or ambiguous requested entry fails that stage rather than being misreported as the first executable transformation. Producing a Partial Explore Report does not change failure into command success.
