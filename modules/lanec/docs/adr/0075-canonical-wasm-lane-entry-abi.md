...
Runtime-import adapters expose the same Lane entry ABI. Their environment must be zero. The adapter uses the runtime symbol registry to convert typed or erased Lane operands to the physical host import signature under module namespace `"lane.runtime.v1"`. `Int`, `Double`, `Bool`, and `Unit` use natural Wasm scalar shapes; a String argument expands to `(bytes_ptr:i32, byte_length:i32)`, and a String result is one owned `string_ref:i32`. A failing import records a structured fatal result and terminates the single-shot instance under ADR 0140; the adapter does not install recovery cleanup.
