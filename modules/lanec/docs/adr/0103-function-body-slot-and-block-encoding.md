...
Function inputs follow the slot table. `environment_slot_plus_one:u32le` uses zero for no environment and `N` for `SlotId = N - 1`. A nonzero environment must identify an `I32 + OwnedRef` slot. Representation-evidence inputs are encoded as `witness_count:u32le` followed by that many `SlotId:u32le` values. Their complete shapes come from the slot table and current Callable ABI; a layout constructor is an `I64 + OwnedCallable` input, while a first-order layout remains `I32 + Trivial`. User inputs use `user_parameter_count:u32le` followed by ordered SlotIds. Environment, evidence, and user-input SlotIds must be pairwise distinct.
