<!-- Managed by Milky2018. Everyone else don't edit! -->

- [ ] enum, struct, effect operation use `auto` argument
- [ ] more `to_string` builtins
- [ ] all readme.md should not be manually newlines 
- [ ] `derive` should be considered by `lane lsp`, type inlay, argument inlay and so on
- [ ] inlay: if there is only one argument and it is `auto`, it will still be a inlayed comma
- [ ] 
