...
A RuntimeImport entry contains `abi_major:u32le`, a counted parameter-kind array, one result-kind tag, `symbol_length:u32le`, and exact symbol bytes. Runtime imports are implicitly no-context and have zero layout-witness parameters. Parameter and result kinds use the closed direct-value domain `Unit`, `Bool`, `Int`, `Double`, `String`, and `Opaque`; `Unit` is valid only as the result. Symbols are nonempty case-sensitive ASCII byte strings without NUL.

After whole-program optimization and final body formation, the linker emits bytecode bodies in final deterministic body-list order. It deduplicates runtime imports by their complete descriptor and appends them sorted lexicographically by symbol bytes, then ABI major, parameter kinds, and result kind. FunctionId assignment is deterministic for the same compiler version, exact inputs, options, and selected entry, but is not stable across compiler, optimization, or linked-program changes.
...
- Runtime import descriptors serialize complete direct-value parameter and result kinds.
- Every source External Type erases to the single `Opaque` ABI kind.
