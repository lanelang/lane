...
A lifted function with free representation parameters stores their LayoutIds in the environment even when it has no value captures. Those free witnesses are loaded locally on entry and are not listed in the function body's explicit witness inputs. Direct generic captures use `I64 + OwnedErased` members whose witness ordinal names the corresponding stored LayoutId; borrowing projection returns both payload and witness.
...
- Free layout witnesses alone require an environment and do not alter explicit witness arity.
- Direct generic captures project their stored witness with the erased payload.
