...
A function body encodes its result ABI after its inputs. `Unit` is tag `0x01`. A value result is tag `0x02` followed by the ordinary representation tag and cleanup tag. Return sources, direct-call destinations, statically known tail targets, and runtime-import result projections must agree with this complete ABI.

Persisted Lane programs use the enclosing linked-program artifact schema as their compatibility boundary. The complete result-ABI format raises `linked_program_schema_version` to 9. A loader rejects older linked-program artifacts before invoking the bytecode decoder, and users regenerate them with the matching compiler. A raw bytecode section exchanged outside that container has no compatibility guarantee and requires a matching LoisVM producer and consumer.
...
- Linked-program schema version 9 identifies the current persisted bytecode payload.
