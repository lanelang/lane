...
`Basic.Io` exports platform-library operations rather than compiler-owned host policy. Its WASIP1 `println` writes through `wasi_snapshot_preview1.fd_write`, while `panic : (String) -> Basic.Data.Void.Void ! { Io, Panic }` writes to standard error, requests `proc_exit(1)`, and finally invokes the private compiler intrinsic `%abort : () -> Void ! Panic` as a non-returning fallback. `%abort` owns only platform-independent terminal control and lowers to fieldless `Fatal` and WebAssembly `unreachable`; it creates no import. A selected executable entry has zero parameters, returns `Unit`, and is validated against an explicit execution profile. The Lane CLI profile admits `Io`, `Panic`, and closed External Effects; residual algebraic or open effects are rejected. Runtime-import resolution and invocation failures remain fatal execution errors rather than Lane effects, but they are distinct from intentional source `Fatal` execution.
