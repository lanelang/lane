...
elaborator, Buslane checker, command line tool, and Wasm runtime.
...
- `runtime/*.lane` should compile but terminate with the documented nonzero
  process outcome.
- `projects/*` contains persistent multi-module fixtures for separate
  compilation, interface loading, linking, and execution.
...
Every single-source example belongs in `fixtures/lane-run.json`. The examples
checker rejects unregistered files. Diagnostic expectations record
