...
Slot-table order implicitly defines zero-based `SlotId` values. Each slot entry records one fixed scalar representation tag and cleanup-category tag. V1 representation tags are `I32`, `I64`, and `F64`; `Unit` occupies no slot. Physical slot allocation may reuse a slot only for compatible logical values. The metadata contains representation and cleanup information needed by typed Wasm lowering and explicit ownership operations, not complete Lane source types or compiler-private borrow regions.
