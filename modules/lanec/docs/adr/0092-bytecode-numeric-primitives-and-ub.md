...
Signed division requires a nonzero divisor and excludes `MIN_INT / -1`. Signed remainder requires a nonzero divisor; `MIN_INT % -1` produces zero. Violating an arithmetic primitive precondition is undefined Lane behavior. Execution backends may trap directly as a defensive implementation choice; such traps are not private fatal exceptions, do not run generated ownership cleanup, and invalidate the current interpreter or Wasm instance.
...
- Invalid signed division has undefined Lane behavior and may trap without ARC cleanup.
